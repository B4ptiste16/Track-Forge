...
function mesh(name: string): MeshData {
  return { name, vertices: [], faces: [], colors: [] };
}

// Push a vertical quad (two posts a->b, z0..z1) coloured `col`, facing `dir`.
function paintQuad(m: MeshData, a: Vec3, b: Vec3, z0: number, z1: number, col: Vec3, dir: Vec3): void {
...
// Axis-aligned box (poles, pillars): 4 sides + top, single colour.
function paintBox(m: MeshData, cx: number, cy: number, z0: number, z1: number, half: number, col: Vec3): void {
  ...
  const base = m.vertices.length;
  for (const [x, y] of c) { m.vertices.push([x, y, z1]); m.colors!.push(col); }
  addQuadUp(m.vertices, m.faces, base, base + 1, base + 2, base + 3);
}

// Stepped bleacher following the given samples on one side of the road.
// Seat rows get tricolore bands along their length.
function buildStand(
  out: MeshData,
  ...
  for (let n = 0; n < idx.length - 1; n++) {
    ...
    for (let k = 0; k < STEPS; k++) {
      ...
      const zHi = zLo + RISE;
      // riser (vertical, faces the track)
      paintQuad(out, at(i, oIn), at(j, oIn), zLo, zHi, [0.42, 0.44, 0.48], inward);
      // tread (seats) — coloured band
      const b = out.vertices.length;
      ...
      for (let k2 = 0; k2 < 4; k2++) out.colors!.push(col);
      addQuadUp(out.vertices, out.faces, b, b + 1, b + 3, b + 2);
    }
    // back wall
    const oBack = off0 + STEPS * DEPTH;
    paintQuad(out, at(i, oBack), at(j, oBack), samples[i].pos[2], samples[i].pos[2] + STEPS * RISE,
...
export function buildDecor(
  ...
  if (t1) {
    ...
    if (idx.length >= 2) {
      ...
      for (let b3 = 0; b3 < 3; b3++) {
        ...
        paintQuad(flag, a, b, zBot, zTop, TRICOLORE[b3], inward);
      }
      out.push(pole, flag);
    }
  }
  ...
  if (straights.length) {
    ...
    if (idx.length >= 4) {
      ...
      if (sub.length >= 2) {
        ...
        buildStand(stand, samples, sub, side, clearOff(mid, side));
      }
    }
  }
  if (t1) {
    ...
    if (idx.length >= 4) {
      const outside: 'left' | 'right' = t1.dir === 'left' ? 'right' : 'left';
      const mid = idx[Math.floor(idx.length / 2)];
      buildStand(stand, samples, idx, outside, clearOff(mid, outside) + 9);
    }
  }
  if (stand.faces.length) out.push(stand);
  ...
  paintBox(arch, pl[0], pl[1], pl[2], pl[2] + H, 0.35, BLANC);
  paintBox(arch, pr[0], pr[1], pr[2], pr[2] + H, 0.35, BLANC);
  ...
  for (let b3 = 0; b3 < 3; b3++) {
    ...
    paintQuad(arch, a, b, zTop - BANNER, zTop, TRICOLORE[b3], fwd);
    paintQuad(arch, a, b, zTop - BANNER, zTop, TRICOLORE[2 - b3], [-fwd[0], -fwd[1], 0]);
  }
  out.push(arch);

  return out.filter((m) => m.faces.length > 0);
}
